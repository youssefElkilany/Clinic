...
const avalaibleSlots = async(req,res,next)=>{
...
// const registerSlot = asyncHandler( async(req,res,next)=>{
...
//         console.log(result.rows[0].doctorid);
...
//     console.log("da5el query b3d slot is registered")
...
const registerSlot = asyncHandler(async (req, res, next) => {
    ...
    client.query(query6, (err, result) => {
        ...
        if (flag) {
            ...
            // Slot is already registered
            return res.json({ message: "slot is already registered" });
        }
    });
});
...
const viewReservation = async(req,res,next)=>{
    ...
    const userId = req.user[0].id // nseeb deh nst5dm asynchandler
...
module.exports = {
    avalaibleSlots,registerSlot,viewReservation,updateReservation,cancelReservation,allReservation
}
